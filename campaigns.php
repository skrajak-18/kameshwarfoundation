...
// Fetch campaigns along with their total raised amount and donor count
// Changed ORDER BY to sort by raised amount descending, so highest funded campaigns show first
$sql = "SELECT c.*, 
        ...
        GROUP BY c.id ORDER BY raised DESC, c.created_at DESC";
$campaigns = $conn->query($sql);

// Fetch ONLY categories that have at least one active campaign
$cat_sql = "SELECT cc.name, cc.icon FROM campaign_categories cc INNER JOIN campaigns c ON cc.name = c.category GROUP BY cc.name";
$active_categories = $conn->query($cat_sql);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Our Campaigns | Kameshwar Foundation</title>
    ...
    <style>
        ...
        .btn-donate { background: linear-gradient(45deg, #d9534f, #ff6b6b); color: white; border-radius: 30px; padding: 10px 25px; font-weight: bold; text-transform: uppercase; letter-spacing: 1px; box-shadow: 0 4px 15px rgba(217, 83, 79, 0.4); transition: all 0.3s ease; border: none;}
        .btn-donate:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(217, 83, 79, 0.6); color: white; }
        
        .page-header { background: linear-gradient(rgba(17, 26, 47, 0.8), rgba(17, 26, 47, 0.8)), url('img/h3.jpeg') center/cover no-repeat; padding: 80px 0; color: white; text-align: center; }
        
        /* SCROLLABLE CATEGORIES WITH ARROWS */
        .category-wrapper { position: relative; display: flex; align-items: center; margin-bottom: 30px; gap: 10px;}
        .category-scroll { display: flex; gap: 15px; overflow-x: auto; scrollbar-width: none; scroll-behavior: smooth; flex-grow: 1; padding: 5px; }
        .category-scroll::-webkit-scrollbar { display: none; }
        
        .scroll-arrow { background: white; border: 2px solid var(--primary-blue); color: var(--primary-blue); width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: 0.3s; flex-shrink: 0; box-shadow: 0 3px 10px rgba(0,0,0,0.1); }
        .scroll-arrow:hover { background: var(--primary-blue); color: white; }

        .cat-btn { background: white; border: 2px solid #e0e5ec; padding: 10px 25px; border-radius: 30px; font-weight: 600; color: #555; white-space: nowrap; cursor: pointer; transition: 0.3s; display: flex; align-items: center; gap: 8px; }
        .cat-btn:hover, .cat-btn.active { background: var(--primary-orange); color: white; border-color: var(--primary-orange); box-shadow: 0 4px 10px rgba(255, 127, 0, 0.2);}

        /* Campaign Cards */
        .camp-card { background: white; border-radius: 15px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.08); transition: 0.3s; height: 100%; display: flex; flex-direction: column; }
...
<body>
    ...
    <header class="page-header">
        <div class="container">
            <h1>Our Campaigns</h1>
            <p class="fs-5 mt-3">Choose a cause close to your heart and make a difference today.</p>
        </div>
    </header>
    ...
    <section class="py-5">
        <div class="container">
            ...
            <div class="category-wrapper">
                <div class="scroll-arrow" onclick="scrollCats(-200)"><i class="fas fa-chevron-left"></i></div>
                
                <div class="category-scroll" id="catScroll">
                    <div class="cat-btn active" onclick="filterCamp('All', this)"><i class="fas fa-th-large"></i> All Causes</div>
                    <?php 
                    if($active_categories->num_rows > 0) {
                        while($cat = $active_categories->fetch_assoc()) {
                            echo "<div class='cat-btn' onclick=\"filterCamp('{$cat['name']}', this)\"><i class='{$cat['icon']}'></i> {$cat['name']}</div>";
                        }
                    }
                    ?>
                </div>

                <div class="scroll-arrow" onclick="scrollCats(200)"><i class="fas fa-chevron-right"></i></div>
            </div>
            ...
            <div class="row g-4" id="campaignList">
                ...
                ?>
                <div class="col-lg-4 col-md-6 camp-item" data-category="<?php echo $camp['category']; ?>">
                    <div class="camp-card">
                        ...
                        <div class="camp-body">
                            ...
                            <div class="stats-row">
                                ...
                                <div class="stat-box">
                                    ...
                                    <div class="stat-val">₹<?php echo number_format($raised); ?></div>
                                </div>
                                <div class="stat-box">
                ...
                <?php 
                    ...
                    echo "<div class='col-12 text-center py-5'><h4 class='text-muted'>No active campaigns found.</h4></div>";
                endif;
                ?>
            </div>
        </div>
    ...
    <script>
        ...
        function scrollCats(amount) {
            document.getElementById('catScroll').scrollBy({ left: amount, behavior: 'smooth' });
        }

        // Filtering Logic
        function filterCamp(cat, btn) {
            document.querySelectorAll('.cat-btn').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');

            let items = document.querySelectorAll('.camp-item');
            items.forEach(item => {
